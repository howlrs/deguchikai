...
impl Default for Op {
    fn default() -> Self {
        let font_filepath = utils::target_path_from_env("DEFAULT_FONT_PATH");

        if !font_filepath.exists() {
            panic!("font file not found: {:?}", font_filepath);
        }

        // back slash to foward slash
        let font_filepath = font_filepath
            .to_string_lossy()
            .into_owned()
            .replace('\\', "/");
...
impl Op {
    ...
    pub fn create_filter_complex(&self) -> String {
        ...
        //
        let mut file = std::fs::File::create("filter_complex.txt").unwrap();
        file.write_all(args.join("").as_bytes()).unwrap();
        file.flush().unwrap();
